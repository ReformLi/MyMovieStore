 ...
 * 模板与农民TV相同（stui模板）
 */
class NiuerVideoSource(
    client: OkHttpClient = defaultClient(),
    cacheRepository: ApiCacheRepository? = null,
    rateLimiter: RequestRateLimiter = RequestRateLimiter("NE", 3_000L, 3)
) : CrawlerVideoSource(client, cacheRepository, rateLimiter) {

    override val sourceId = "crawler_niuer"
    override val cachePrefix = "niuer"
    override val rateLimiterTag = "NE"
    override val logTag = "NiuerVideoSource"
    ...
    override fun parseVideoDetail(doc: Document, detailUrl: String): CrawlerVideoDetail {
        ...
        // 灵活查找播放列表容器
        val playlistPanel = doc.select(".stui-pannel.b.playlist").first()
            ?: doc.select(".stui-pannel-box.b.playlist").first()

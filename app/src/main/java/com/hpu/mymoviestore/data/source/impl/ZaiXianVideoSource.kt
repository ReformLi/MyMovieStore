 ...
 * 模板与农民影视相同（stui模板）
 */
class ZaiXianVideoSource(
    client: OkHttpClient = defaultClient(),
    cacheRepository: ApiCacheRepository? = null,
    rateLimiter: RequestRateLimiter = RequestRateLimiter("ZX", 3_000L, 3)
) : CrawlerVideoSource(client, cacheRepository, rateLimiter) {

    override val sourceId = "crawler_zaixian"
    override val cachePrefix = "zaixian"
    override val rateLimiterTag = "ZX"
    override val logTag = "ZaiXianVideoSource"
    ...
    override fun parseVideoDetail(doc: Document, detailUrl: String): CrawlerVideoDetail {
        ...
        // 1. 标题：从 h1.title 提取，包含评分标签需要移除
        val titleElement = doc.select(".stui-content__detail h1.title").first()
        var title = ""
        if (titleElement != null) {
            val clone = titleElement.clone()
            clone.select(".score").remove()
            title = clone.text().trim()
        }
        if (title.isBlank()) {
            title = doc.select(".stui-content__detail h1.title").first()?.text()?.trim()
                ?.replace(Regex("\\s+\\d+\\.\\d+$"), "")?.trim() ?: ""
        }
        ...
        // 5. 简介：从 .desc 或 #desc 提取
        val descElement = doc.select(".desc").first()
        if (descElement != null) {
        ...
        if (description.isBlank()) {
            description = doc.select("#desc .stui-pannel_bd .col-pd").first()?.text()?.trim().orEmpty()
        }

        // 6. 播放线路和集数
        val lines = mutableListOf<PlayLine>()

        // 每个播放线路在一个 .stui-pannel.b.playlist 中
        ...
        for (panel in playlistPanels) {
            // 线路名称：从 .stui-pannel_hd .title 获取
            var lineName = panel.select(".stui-pannel_hd .title").first()?.text()?.trim().orEmpty()
            if (lineName.isBlank()) continue
            // 清理线路名称，去掉《》等
            lineName = lineName.replace(Regex("[《》]"), "").trim()
        ...
        // 兜底方案：如果上面没找到，直接查找 .stui-content__playlist 的父容器
        if (lines.isEmpty()) {
            val fallbackPanel = doc.select(".stui-content__playlist").first()?.closest(".stui-pannel-box")
            if (fallbackPanel != null) {
                val episodeLinks = fallbackPanel.select(".stui-content__playlist li a")
                val episodes = episodeLinks.mapNotNull { a ->
                    val episodeTitle = a.text().trim()
                    val playPageUrl = a.attr("abs:href")
    ...
    override fun extractRealVideoUrl(scriptContent: String): String? {
        ...
        // ===== 新增：提取 const vid = '...' =====
        // 匹配格式：const vid = 'https://.../index.m3u8';
        val vidRegex = Regex("""const\s+vid\s*=\s*'([^']+)'""")
        ...
        if (match != null) {
            ...
            if (videoUrl.isNotBlank()) {
                Log.d(logTag, "✅ 从 const vid 提取到播放地址: $videoUrl")
                return videoUrl
            }
        }
    ...
    override suspend fun fetchVideoUrlByPlayPageUrl(playPageUrl: String): Result<String> = withContext(
        ...
        try {
            // 直接返回测试地址
            val testUrl = "https://hn.bfvvs.com/play/xboNMEXa/index.m3u8"
            Log.d(logTag, "强制返回测试地址: $testUrl")
